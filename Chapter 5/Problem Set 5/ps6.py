...
x = Message('L zdqw wr jr wr wkh vwruh')
x.apply_shift(23)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        totalWords = []
        for s in range(1,27):
            message = Message.apply_shift(self, 26-s)
            messageList = message.split(' ')
            for word in messageList:
                wordCount = 0
                if is_word(self.valid_words, word) == True:
                    wordCount += 1
            totalWords.append(wordCount)
        maxIndex = totalWords.index(max(totalWords))
        return (26 - maxIndex - 1, Message.apply_shift(self, 26-maxIndex-1))
